...
class TestCallback(Callback):
    ...
    def on_train_end(self, logs=None):
        ...
        for _class in range(self.classes):
            ...
            auc_keras = auc(fpr_roc, tpr_roc)
            plt.figure(1)
            plt.plot([0, 1], [0, 1], 'k--')
            plt.plot(fpr_roc, tpr_roc, label=f'{self.class_names[_class]} (area = {auc_keras:.3f})')
            ...
            plt.figure(3)
            plt.plot([0, 1], [0, 1], 'k--')
            plt.plot(det_fpr, det_fnr, label=f'{self.class_names[_class]} (area = {auc_keras:.3f})')
            plt.xlabel('False positive rate')
            plt.ylabel('True positive rate')
