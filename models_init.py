...
def model_struct(args):
    conv_nodes = np.asarray([2, 3, 3.5, 4]) * args['conv_layers']
    conv_nodes = conv_nodes.astype(np.int)
    dense_nodes = np.asarray([1, 2]) * args['dense_layers']
    merge_nodes = np.asarray([1, 2, 2.5]) * args['merge_layers']
    merge_nodes = merge_nodes.astype(np.int)

    act = {'swish': tf.keras.activations.swish,
    ...
    inc_avrg = conv2d_norm(input_tensor=inc_avrg, nodes=conv_nodes[0], activation=act, kernel_size=1,
                           dropout=args['dropout'], kernel_regularizer=rglzr, initializer=init)
...
# --------------------------------================ Tabular data =================--------------------------------- #
    if not args['no_clinical_data']:
        ...
        clinical_data_1 = tf.keras.layers.Dense(dense_nodes[1], activation=act, kernel_regularizer=rglzr, kernel_initializer=init, bias_initializer=init)(clinical_data_input)
        ...
        clinical_data_2 = tf.keras.layers.Dense(dense_nodes[0], activation=act, kernel_regularizer=rglzr, kernel_initializer=init, bias_initializer=init)(clinical_data_1)
        ...
        clinical_data_3 = tf.keras.layers.Dense(dense_nodes[0], activation=act, kernel_regularizer=rglzr, kernel_initializer=init, bias_initializer=init)(clinical_data_con)
    ...
    common = tf.keras.layers.Dense(merge_nodes[2], activation=act, kernel_regularizer=rglzr, kernel_initializer=init,
    ...
    common = tf.keras.layers.Dense(merge_nodes[1], activation=act, kernel_regularizer=rglzr, kernel_initializer=init,
                                   bias_initializer=init)(common)
    common = tf.keras.layers.LayerNormalization()(common)
    # common = tf.keras.layers.Dropout(rate=args['dropout'], seed=seed)(common)
    common = tf.keras.layers.Dense(merge_nodes[0], activation=act, kernel_regularizer=rglzr, kernel_initializer=init, bias_initializer=init)(common)
    # common = tf.keras.layers.LayerNormalization()(common)
    # common = tf.keras.layers.Dense(16, activation=activation, kernel_regularizer=rglzr)(common)
    output = tf.keras.layers.Dense(len(TASK_CLASSES[args['task']]), activation='softmax', kernel_regularizer=rglzr, kernel_initializer=init, bias_initializer=init, name='class')(common)
