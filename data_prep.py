...
def get_val_test_dataset(args, dataset, dirs):
    ...
    image_path_ds = tf.data.Dataset.from_tensor_slices(image_path)
    images_ds = image_path_ds.map(_read_images, num_parallel_calls=tf.data.AUTOTUNE, deterministic=True)
    image_path_ds = tf.data.Dataset.from_tensor_slices(image_path).batch(20 * args['batch_size'] * args['gpus'], num_parallel_calls=tf.data.AUTOTUNE, deterministic=True)
    images_ds = images_ds.batch(20 * args['batch_size'] * args['gpus'], num_parallel_calls=tf.data.AUTOTUNE, deterministic=True)
    onehot_features_ds = tf.data.Dataset.from_tensor_slices(onehot_features).batch(20 * args['batch_size'] * args['gpus'], num_parallel_calls=tf.data.AUTOTUNE, deterministic=True)
    onehot_label_ds = tf.data.Dataset.from_tensor_slices(onehot_label).batch(20 * args['batch_size'] * args['gpus'], num_parallel_calls=tf.data.AUTOTUNE, deterministic=True)
