...
def resize_cvt_color(sample, args):
    ...
    image = cv2.resize(src=image, dsize=(int(args['image_size']), int(args['image_size'])), interpolation=cv2.INTER_NEAREST_EXACT)
...
def check_create_dataset(args, force=False):
    ...
    if not os.path.exists(args['dir_dict']['image_folder']) or force is True:
        ...
        samples = pd.read_csv(args['dir_dict']['data_csv']['train']).append(pd.read_csv(args['dir_dict']['data_csv']['val'])).append(pd.read_csv(args['dir_dict']['data_csv']['val']))
        if mp.cpu_count() > 32:
            threads = 32
        else:
            threads = mp.cpu_count() * 2
        pool = mp.Pool(threads)
        pool.starmap(resize_cvt_color, [(sample, args) for idx, sample in samples.iterrows()])
        pool.close()
        print('Done!')
    else:
